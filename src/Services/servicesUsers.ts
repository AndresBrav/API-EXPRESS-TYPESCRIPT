...
export const getOneUser = async (id: string, loginUser: string): Promise<UsersInstance | null> => {
    const tipo = await User.findOne({
...
export const updateUser = async (
    ...
    if (typeof login === 'string') {
        ...
        return new ResError(respCode.BAD_REQUEST, respPhrase.INCORRECT_FIELD.val, null);
    }

    if (typeof clave === 'string') {
        fieldstoUpdate.clave = bcrypt.hashSync(clave, 10);
    } else {
        return new ResError(respCode.BAD_REQUEST, respPhrase.INCORRECT_FIELD.val, null);
    }
    if (typeof sts === 'string') {
        fieldstoUpdate.sts = sts;
    } else {
        return new ResError(respCode.BAD_REQUEST, respPhrase.INCORRECT_FIELD.val, null);
    }
